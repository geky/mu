...
var_t mu_exec(fn_t *f, tbl_t *args, tbl_t *scope, eh_t *eh) {
    ...
    mu_on_err_begin (eh) while (1) {
        ...
        switch (*pc++ >> 3) {
            case OP_VAR:    *--sp = f->vars[arg(pc)]; pc += 2;                                  break;
            case OP_FN:     *--sp = vfn(f->fns[arg(pc)], scope); pc += 2;                       break;
            case OP_NIL:    *--sp = vnil;                                                        break;
            case OP_TBL:    *--sp = vtbl(tbl_create(0, eh));                                     break;
            case OP_SCOPE:  *--sp = vtbl(scope);                                                 break;
            case OP_ARGS:   *--sp = vtbl(args);                                                  break;
