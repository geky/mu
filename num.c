    ...
    if (n == 0) {
        ...
        if (isexp) {
            *pos++ = 'e';
            ...
            if (exp > 100) *pos++ = mu_toascii((int_t)exp / 100);
            if (exp > 10)  *pos++ = mu_toascii(((int_t)exp / 10) % 10);
            if (exp > 1)   *pos++ = mu_toascii(((int_t)exp / 1) % 10);
        }
